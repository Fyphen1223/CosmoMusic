...
const radioUrl = [
	...
	"https://mpc2.mediacp.eu/stream/natureradiosleep",
	"http://air.radioart.com/fNature.mp3",
	"http://orion.shoutca.st:8157/stream",
	"https://drive.uber.radio/uber/asmrmouthsounds/icecast.audio",
	"https://drive.uber.radio/uber/asmrtapping/icecast.audio",
...
var voskModel = "";
if (config.config.voiceRecognition) {
	voskModel = new vosk.Model(voskModelPath);
	vosk.setLogLevel(0);
}
	...
	player.on(AudioPlayerStatus.Playing, async (meta) => {
		const embed = new EmbedBuilder()
			...
			.addFields(
				{
					name: "Author",
					value: audio[guildId]["artist"],
					inline: true,
				},
				{ name: "Title", value: audio[guildId]["title"], inline: true },
				{
					name: "Duration",
					value: `${util.formatTime(Math.floor(audio[guildId]["seek"] + meta.resource.playbackDuration / 1000))}/${util.formatTime(meta.resource.metadata.length)}`,
					inline: true,
				},
				{
					name: "Started by",
					value: `${meta.resource.metadata.user}`,
					inline: true,
				},
				{
					name: "Volume",
					value: `${Math.floor(audio[guildId]["volume"] * 10) * 10}%`,
					inline: true,
				},
				{
					name: "Position",
					value: `${audio[guildId]["index"] + 1}/${audio[guildId]["queue"].length}`,
					inline: true,
				}
			)
		...
		const msg = await audio[guildId]["previousChannel"].send({
			embeds: [embed],
...
async function startPlay(guildId, interaction, msg) {
	...
	try {
		...
		interaction.editReply("I'm sorry, but currently the URL is not available.");
		audio[guildId]["queue"] = [];
		return;
	}
	audio[guildId]["seekStream"] = ytU.downloadRaw(queueUrl);
...
async function fetchMany(channel, options = { limit: 50 }) {
	...
	if (typeof options.around === "string") {
		...
		return util.array2Collection([messages, ...(await Promise.all([backward, forward]))].flatMap((e) => [...e.values()]));
	}
	let temp;
...
let streamInt = 1;
function convertToLeadingZeroes(num, numDigits) {
...
client.on("messageCreate", async (msg) => {
	if (msg.author.tag === "fyphen#0" && msg.content === "stop") {
		await msg.react("👍");
		console.log("Process exited with the admin's command.");
		process.exit();
	}
	...
	if (ytdl.validateURL(msg.content)) {
		const stream = await ytU.downloadForView(msg.content);
		stream.pipe(await fs.createWriteStream(`./rec/${convertToLeadingZeroes(streamInt, 3)}.wav`));
		stream.on("end", async function () {
			await msg.reply({
				files: [`./rec/${convertToLeadingZeroes(streamInt, 3)}.wav`],
			});
			streamInt++;
		});
		return;
...
function convertToWavStream(input) {
	const encoder = new prism.WavEncoder({ channels: 1, sampleRate: 48000 });
	input.pipe(encoder);
	return encoder;
}
client.on("interactionCreate", async (interaction) => {
	if (interaction.commandName === "ping") {
		...
		const resultEmbed = new EmbedBuilder()
			...
			.addFields(
				...
				{
					name: "Message Delay",
					value: `${rep.createdTimestamp - interaction.createdTimestamp}ms`,
					inline: true,
				},
				{ name: "Status", value: `${speed}`, inline: true }
			);
	...
	if (interaction.commandName === "clear") {
		try {
			...
			await interaction.reply({
				content: `Clear message number of ${num}`,
				ephemeral: true,
			});
		} catch (err) {
			interaction.reply({
				content: "An error occurred when try to delete message",
				ephemeral: true,
			});
		}
	}
	if (interaction.commandName === "whois") {
		try {
			...
			interaction.reply({
				content: "I'm sorry, Error with whois command.",
				ephemeral: true,
			});
		}
	}
	if (interaction.commandName === "screenshot") {
		...
		(async () => {
			...
			try {
				...
				await browser.close();
				await interaction.editReply({
					content: "Screenshot Done",
					files: [img],
				});
			} catch (err) {
				console.log(err.stack);
				await interaction.editReply({
					content: "Cannot take screenshot",
					ephemeral: true,
				});
			}
		})();
	...
	if (interaction.commandName === "pause" || interaction.customId === "pause") {
		...
		if (!interaction.member.voice.channelId) {
			await interaction.editReply("Please connect voice channel.");
			return;
		}
		if (interaction.member.voice.channelId !== audio[guildId]["channel"]) {
			await interaction.editReply("You are not in my voice channel.");
			return;
		}
		try {
	...
	if ((interaction.commandName === "queue" && !interaction.isAutocomplete()) || (interaction.customId === "queue" && !interaction.isAutocomplete()) || interaction.customId === "addR") {
		await interaction.deferReply();
		const guildId = interaction.guild.id.toString();
		if (!audio[guildId]) guildAudioListInit(guildId);
		if (interaction.customId === "queue") {
			...
			if (audio[guildId]["queue"].length === 0) {
				...
				content = "";
				let i = 1;
				while (i <= audio[guildId]["queue"].length) {
					content = content + `\n${i}: ${audio[guildId]["queue"][i - 1]}`;
					i++;
				}
		...
		if (interaction.options.getSubcommand() === "display") {
			...
			try {
				...
				await interaction.editReply({
					content: "Cannot remove music from queue",
					ephemeral: true,
				});
			}
	...
	if (interaction.commandName === "youtube") {
		...
		if (interaction.options.getSubcommand() === "search") {
			...
			const embed = new EmbedBuilder().setTitle("Search Results").setColor(config.config.color.info);
			if (res.length === 0) {
			...
			while (i < 5 && i < res.length) {
				embed.addFields({
					name: `${res[i].title + " | " + res[i].channel.name}`,
					value: res[i].link,
					inline: false,
				});
				i++;
	...
	if (interaction.commandName === "tts") {
		...
		if (interaction.member.voice.channelId === client.voice.channelId) {
			...
			await interaction.editReply({
				content: `${content} has been played.`,
			});
			return;
		}
		if (interaction.member.voice.channelId) {
	...
	if (interaction.commandName === "config") {
		...
		if (interaction.options.getSubcommand() === "autoreplay") {
			...
			const embed = new EmbedBuilder().setTitle(`Set your guild's auto-replay to ${interaction.options.getBoolean("autoreplay").toString().toUpperCase()}`);
			await interaction.editReply({ embeds: [embed] });
		}
		if (interaction.options.getSubcommand() === "useytm") {
			...
			const embed = new EmbedBuilder().setTitle(`Set your guild's auto-use ytm to ${interaction.options.getBoolean("bool").toString().toUpperCase()}`);
			await interaction.editReply({ embeds: [embed] });
		}
	}
	if (interaction.customId === "volumeUp" || interaction.customId === "volumeDown") {
		...
		if (interaction.customId === "volumeUp") {
			if (currentVolume >= 3.0) {
				await interaction.editReply("You cannot set volume higher than 300%.");
				return;
			} else {
				...
				const embed = new EmbedBuilder().setColor(config.config.color.info).setTitle(`Set volume to ${Math.floor(currentVolume * 10 + 1) * 10}%`);
				await interaction.editReply({ embeds: [embed] });
				return;
			}
		} else {
			if (currentVolume <= 0.1) {
				await interaction.editReply("You cannot set volume to 0.");
				return;
			} else {
				audio[guildId]["resource"].volume.setVolume(Math.floor(currentVolume * 10 - 1) / 10);
				audio[guildId]["volume"] = Math.floor(currentVolume * 10 - 1) / 10;
	...
	if (interaction.commandName === "suggest") {
		if (interaction.options.getSubcommand() === "yesorno") {
			await interaction.deferReply();
			const { body } = await request("https://yesno.wtf/api");
			const ans = await body.json();
			const embed = new EmbedBuilder().setColor(config.config.color.info).setTitle(ans.answer.toUpperCase()).setDescription(ans.answer.toUpperCase()).setImage(ans.image);
	...
	if (interaction.commandName === "playlist") {
		...
		if (interaction.options.getSubcommand() === "save") {
			...
			await interaction.editReply({
				content: `Added current queue to saved queue. Items:${music[0][guildId]["queue"].length} ID: ${music[0][guildId]["savedQueue"]}`,
			});
		}
	}
	if (interaction.commandName === "play" && !interaction.isAutocomplete()) {
		...
		if (interaction.options.getBoolean("autoreplay") === true) {
			...
			if (audio[guildId]["autoReplay"]) audio[guildId]["autoReplay"] = false;
		}
		audio[guildId]["previousChannel"] = interaction.channel;
		...
		audio[guildId]["connection"].receiver.speaking.on("start", (userId) => {
			if (!config.config.voiceRecognition) return;
			...
			var stream = "";
			const writer = voiceStream.pipe(decoder);
			var wavStream = convertToWavStream(stream).pipe(fs.createWriteStream("./hi.wav"));
			//stream
			writer.on("finish", () => {
		...
		try {
			...
			const playlitAddEmbed = new EmbedBuilder().setColor(config.config.color.info).setAuthor({
				name: ` | ✅ - Added ${playlist.items.length} items to the queue.`,
				iconURL: `${interaction.user.avatarURL({})}`,
			});
			...
			if (ytdl.validateURL(url)) {
				...
				if (!audio[guildId]["id"]) {
					audio[guildId]["queue"].push(url);
					const addUrlEmbed = new EmbedBuilder().setColor(config.config.color.info).setAuthor({
						name: ` | ✅ - Added ${url} to the queue.`,
						iconURL: `${interaction.user.avatarURL({})}`,
					});
				...
				try {
					const raw = await ytU.spotifyToYT(url);
					if (!raw) {
						await interaction.editReply("Invalid URL. Couldn't find any song.");
					}
					audio[guildId]["queue"].push(raw);
					const resultEmbed = new EmbedBuilder().setColor(config.config.color.info).setAuthor({
						name: ` | 🔍 Added ${raw} to the queue.`,
						iconURL: `${interaction.user.avatarURL({})}`,
					});
					if (!audio[guildId]["id"]) {
				...
				if (audio[guildId]["useYTM"]) {
					...
					const stat = await api.search(url, "song").then(async (result) => {
						if (result["content"].length === 0) {
							interaction.editReply("There's no music.");
							return "PLAYING";
						}
						const resultEmbed = new EmbedBuilder().setColor(config.config.color.info).setAuthor({
							name: ` | 🔍 Added ${result["content"][0]["name"]} to the queue.`,
							iconURL: `${interaction.user.avatarURL({})}`,
						});
					...
					const resultEmbed = new EmbedBuilder().setColor(config.config.color.info).setAuthor({
						name: ` | 🔍 Added ${result[0]["title"]} to the queue.`,
						iconURL: `${interaction.user.avatarURL({})}`,
					});
					interaction.editReply({ embeds: [resultEmbed] });
					if (!audio[guildId]["id"]) {
		...
		if (interaction.channel) addEventListenerToPlayer(audio[guildId]["player"], guildId);
		await startPlay(guildId, interaction);
		return;
	}
	if (interaction.commandName === "help") {
		await interaction.deferReply();
		const embed = new EmbedBuilder().setColor(config.config.color.info).setTitle("Help Menu").setTimestamp().addFields(
			{ name: "Status", value: "/ping /status", inline: true },
			{
				name: "Music",
				value: "/play /pause /unpause /stop",
				inline: true,
			},
			{
				name: "Queue",
				value: `/ queue display / queue add / queue remove / queue delete `,
				inline: true,
			},
			...
			{
				name: "Radio",
				value: `/ radio play / radio stop`,
				inline: true,
			},
			{
				name: "Search",
				value: `/ youtube search / ncs search`,
				inline: true,
			}
		);
		await interaction.editReply({ embeds: [embed] });
	...
	if (interaction.commandName === "learn") {
		if (interaction.options.getSubcommand() === "speak") {
			...
			if (!interaction.member.voice) {
				await interaction.editReply("Please connect a voice channel!");
				return;
			}
			const connection = joinVoiceChannel({
				channelId: interaction.member.voice.channelId,
...
async function skip(guildId) {
	...
	if (index < audio[guildId]["queue"].length) {
		...
		try {
			audio[guildId]["rawStream"] = ytU.downloadRaw(url);
...
function createFFmpegStream(stream, seek, str) {
	...
	let transcoder = new prism.FFmpeg({
		args: [
			...
			seek || "0",
			"-ab",
			config.config.voice.maxQuality,
			//'-af', `bass=g=${str}`
		],
	});

...
function generatePauseEmbed(queue, guildId, type, config) {
    if (!type || type === 0) {
...
function generateStartEmbed(queue, guildId, type, config) {
    if (!type || type === 0) {
        const current = queue[guildId].queue[queue[guildId].index].data.info;
        const embed = new EmbedBuilder()
            .setColor(config.config.color.info)
...
function generateUnpauseEmbed(queue, guildId, type, config) {
    if (!type || type === 0) {
        const current = queue[guildId].queue[queue[guildId].index].data.info;
        const embed = new EmbedBuilder()
            .setColor(config.config.color.info)
...
module.exports = { generatePauseEmbed, generateStartEmbed, generateUnpauseEmbed };